...
public class SpreadsheetsService {
    ...
    abstract class Request<T> {
        ...
        private final T internalExecute(boolean firstTry) throws IOException, SpreadsheetsException {
            ...
            try {
                ...
                if (firstTry && e.getMessage().equals("401 Token invalid")) {
                    ...
                    throw new SpreadsheetsHttpException(e.getResponse().getStatusCode(), e.getResponse().getStatusMessage());
                }
    ...
    public void createSpreadsheet(String title, boolean hidden) throws IOException, SpreadsheetsException {
        ...
        new Request<Void>() {
            public Void run() throws IOException, XmlPullParserException {
                ByteArrayContent content = ByteArrayContent.fromString(null, builder.toString());
                HttpRequest request = writelyRequestFactory.buildPostRequest(url, content);
                
                request.setEnableGZipContent(false);
                HttpHeaders headers = request.getHeaders();
                headers.setContentType("application/atom+xml");
